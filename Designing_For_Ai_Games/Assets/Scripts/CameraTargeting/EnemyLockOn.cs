...
public class EnemyLockOn : MonoBehaviour
{
    ...
    [Tooltip("Angle_Degree")] [SerializeField] private float maxNoticeAngle = 60;
    ...
    private DefMovement defMovement;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse2) || Input.GetKeyDown(KeyCode.E))
        {
            if (currentTarget)
            {
    ...
    {
        Collider[] nearbyTargets = Physics.OverlapSphere(transform.position, noticeZone, targetLayers, QueryTriggerInteraction.UseGlobal);
    ...
    bool TargetOnRange(){
        Vector3 fixedPlayerPos = transform.position;
        Vector3 fixedEnemyLockPos = pos;
        fixedPlayerPos.y = 0;
        fixedEnemyLockPos.y = 0;

        float dis = Vector3.Distance(fixedPlayerPos, fixedEnemyLockPos);
